...
class Actor(nn.Module):
	...
	def forward(self, x):
		...
		# In DDPG,actor provide an deterministic action choice,then some noise generated by normal dist is then added to this a as exploration. 
		# This normal dist is fixed, used for noise only, while in AC or A2C the actor network provide an std. These two ways are similar but have different
		# interpretations. DDPG: policy network gives deterministic action, AC/A2C gives a probability distribution. 
...
class DDPG(object):
	...
	def update(self):
		...
		for it in range(args.update_iteration):
			...
			# Compute actor loss
			#this is mean of Q for all sampled states.We want to maximize this result as we wish to have maximum returns/Q-values. 
			# though this is different from what we have seen in previous models, it is simple and quites reasonable as well.those actor_loss that includes 'log' are aiming at maximize the V instead of Q. 
			...
			# Soft update insteal of 'hard update'
