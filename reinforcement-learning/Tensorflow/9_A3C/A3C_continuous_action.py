...
print('multiprocessing.cpu_count():'+str(N_WORKERS))
MAX_EP_STEP = 200
MAX_GLOBAL_EP = 4000
...
N_A = env.action_space.shape[0] #1
A_BOUND = [env.action_space.low, env.action_space.high] #[-2,2]
...
class ACNet(object):
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope('critic'):  #能看出这个critic网络只用了state作为input，所以估计的是state value function
            ...
            for ep_t in range(MAX_EP_STEP):
                ...
                if total_step % UPDATE_GLOBAL_ITER == 0 or done:   # update global and assign to local net
                    if done:
                        ...
                        v_s_ = SESS.run(self.AC.v, {self.AC.s: s_[np.newaxis, :]})[0, 0]  #没失败就用V(st,theta(from local critic network))来当作奖励
                    buffer_v_target = []
                    for r in buffer_r[::-1]:    # reverse buffer r 
                        ...
                        #[r1,r2,r3,r4,...,rn]
                        #[rn,rn-1+Gamma*rn,...,r1+Gamma*r2+Gamma^2*r3+...]  
                    ...
                    self.AC.update_global(feed_dict) #更新到global net
                    buffer_s, buffer_a, buffer_r = [], [], [] #清空
                    self.AC.pull_global() #用global net更新local net
...
if __name__ == "__main__":
    ...
    plt.savefig(dirname+'/reward_continous_action.png')
    plt.show()
